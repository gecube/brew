...
module Utils
  ...
  # accumulating serially. Results keep the order of `items` and the first
  # exception is re-raised by `Thread#value`, though later items' blocks may
  # still run to completion first. Only worthwhile when each block spends its
  # time waiting: the GVL serializes Ruby execution.
  sig {
